...
const CakesByFlavours = () => {

    const [cakesByFlavours, setCakesByFlavours] = useState([])
    const dispatch = useDispatch()
    const valueToFind = useSelector((state) => state.value.valueToFind)


    useEffect(() => {
        CakeService.fetchCakeByFlavours(valueToFind).then(data => {
            setCakesByFlavours(data)
        })
    }, [])

    return (
        <div className="cakes-by-flavours">
            {cakesByFlavours.length !== 0
                ?
                <div className="flavourwise-list">
                    < div className="row row-cols-1 row-cols-md-2 g-4" >
                        {cakesByFlavours && cakesByFlavours.map((cake) => {
                            return (
                                <div className="flavourwise-cake" key={cake.id} >
                                    <div className="row">
                                        <div className="col">
                                            {/* Image */}
...
export default CakesByFlavours
